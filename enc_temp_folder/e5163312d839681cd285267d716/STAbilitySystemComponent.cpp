	...
	if (ACharacter* Character = Cast<ACharacter>(Avatar))
	{
		...
		if (Character->IsLocallyControlled() && Character->IsPlayerControlled())
		{
			if(PredictionHandle.IsValid())
				AbilityActivatedCallbacks.Remove(PredictionHandle);
			auto Delegate = FGenericAbilityDelegate::FDelegate::CreateUObject(this, &ThisClass::RegisterNotifyEvent);
			PredictionHandle = AbilityActivatedCallbacks.Add(Delegate);
		}
	}
}
...
void USTAbilitySystemComponent::RegisterNotifyEvent(UGameplayAbility* GA)
{	
	ScopedPredictionKey.NewRejectedDelegate().BindUObject(this, &ThisClass::OnRejected, GA);
	ScopedPredictionKey.NewCaughtUpDelegate().BindUObject(this, &ThisClass::OnCaughtUp,GA);
}

void USTAbilitySystemComponent::OnRejected(UGameplayAbility* GA)
{
...
void USTAbilitySystemComponent::OnCaughtUp(UGameplayAbility* GA)
{
	if (USTGameplayAbility* STGA = Cast<USTGameplayAbility>(GA))
	{
		STGA->OnGameplayAbilityPredictionRejected();
